  ...
  try {
    ...
    if (tokenRes.superadmin || tokenRes.admin) {
      ...
      await transporter.sendMail({
        ...
        to: tokenRes.email,
        text: `Password Reset Verification, use this ${baseURL}/changePassword/${token} link to reset password`,
        html: `<h3>Password Reset Verification </h3> <br /> <a href="${baseURL}/changePassword/${token}">Click here</a> to reset password`,
      });
...
const sendAdminRegistrationEmailLink = async (
  ...
  try {
    await transporter.sendMail({
      ...
      text: `Admin Registration process - Click on this link and set password to become an admin ${baseURL}/register/${token}`,
...
router.post("/registerCustomer", async (req, res) => {
  const data: { uid: string; token: string; email: string } = req.body;
  try {
    try {
